...
namespace Jank.DataStructures.Tree
{
    ...
    public class UniqueChildrenTreeNode<T> : TreeNode<T> where T : IEquatable<T>
    {
        ...
        {
            if (Children.QueryContains(n => { return n.Value.Equals(value); }))
            {
                return null;
            }
        ...
        {
            if (Parent != null && Parent.ChildrenNodes.QueryContains(n => n.Value.Equals(value)))
            {
                return false;
            }
        ...
        public override int Similarity(ITreeNode<T> other, DEquality<ITreeNode<T>> equality)
        {
            ...
            List<ITreeNode<T>> nodes = new List<ITreeNode<T>>(other.ChildrenNodes);
        ...
        public override ITreeNode<T> Union(ITreeNode<T> other, DEquality<ITreeNode<T>> equality, DMerge<T> merge)
        {
            ...
            List<ITreeNode<T>> otherNodesNotInOriginal = new List<ITreeNode<T>>(other.ChildrenNodes);
        ...
        public override ITreeNode<T> Intersection(ITreeNode<T> other, DEquality<ITreeNode<T>> equality)
        {
            ...
            List<ITreeNode<T>> otherChildren = new List<ITreeNode<T>>(other.ChildrenNodes);
        ...
        public override ITreeNode<T> Difference(ITreeNode<T> other, DEquality<ITreeNode<T>> traversalEquality, DEquality<T> deletionEquality)
        {
            ...
            if (ChildCount == 0)
            {
                return deletionEquality(Value, other.Value) ? null : new UniqueChildrenTreeNode<T>(Value);
            }
            ...
            foreach (ITreeNode<T> child in other.ChildrenNodes)
            {
        ...
        void UnsafeAddChild(UniqueChildrenTreeNode<T> parent, UniqueChildrenTreeNode<T> child)
        {
