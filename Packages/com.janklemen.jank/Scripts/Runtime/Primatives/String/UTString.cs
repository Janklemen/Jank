...
using Jank.Primatives.String.CharQueries;
...
namespace Jank.Primatives.String
{
    ...
    public static class UtString
    {
        ...
        {
            int startIndex = original.IndexOf(start, StringComparison.Ordinal);
            if (startIndex == -1) return string.Empty;

            int endIndex = original.IndexOf(end, StringComparison.Ordinal);
        ...
        {
            ScanUntil(ref s, ref index, UtCharQuery.GetOccurenceOfQuery(UtCharQuery.LinuxNewline));
            index++;
        }
        #endregion
