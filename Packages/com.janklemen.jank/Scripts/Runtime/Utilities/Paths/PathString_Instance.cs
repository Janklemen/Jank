...
namespace Jank.Utilities.Paths
{
    /// <summary>
    ...
    {
        const string cIndexLessThanZeroErrorMessage = "Index must be > 0";
        const string cIndexGreaterThanLengthErrorMessage = "Index greater than path length";
        const string cInvalidArgErrorMessage = "Steps must be provided as a path string, so steps separated by / or \\";

        string[] _path; // this is the cached path in steps
        ...
        /// </summary>
        public char Separator { get; set; } = WindowsPathSeparator;
        ...
        public string Extension {
            get
            {
                ...
                int extensionIndex = end.IndexOf(".", StringComparison.Ordinal);
        ...
        public PathString(string path)
        {
            ...
            const string ArgErrorMessage = "The supplied string contains equal number / and \\. This is an invalid path and cannot be used in PathString because the path separator cannot be inferred";
            if (!an.Success)
        ...
        public PathString(params string[] path) : this(Path.Combine(path))
        {
        ...
        {
            const string IndexErrorMessage = "Endindex must be greater than start index";

            if (endIndex <= startIndex) throw new ArgumentException(IndexErrorMessage);
        ...
        /// </summary>
        public Queue<string> AsQueue() => new(_path);

        /// <summary>
        /// Get the path as a stack (the last member is poped each time)
        /// </summary>
        public Stack<string> AsStack() => new(_path);

        /// <summary>
        /// Get the path as a list
        /// </summary>
        public List<string> AsList() => new(_path);
        ...
        public bool CreateIfNotExistsDirectory()
        {
            try
            {
                DirectoryInfo di = new(ToString());

                if (!di.Exists)
                {
                    RemoveAtEnd().CreateIfNotExistsDirectory();
                    di.Create();
                }
            }
            catch (Exception)
        ...
        PathAnalysis Analyze(string path)
        {
            // test the most common char
            int unixTest = path.ToCharArray().QueryCount(c => c == UnixPathSeparator);
            int winTest = path.ToCharArray().QueryCount(c => c == WindowsPathSeparator);

            if (winTest > unixTest)
                return new PathAnalysis(true, WindowsPathSeparator, path.Split(WindowsPathSeparator));
            
            if (unixTest > winTest)
                return new PathAnalysis(true, UnixPathSeparator, path.Split(UnixPathSeparator));

            if (unixTest == 0 && winTest == 0)
                return new PathAnalysis(true, Separator, new[] { path });
